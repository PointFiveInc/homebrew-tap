# typed: false
# frozen_string_literal: true

# This file was generated by GoReleaser. DO NOT EDIT.
class Tokenshift < Formula
  desc "Config-driven CLI proxy that compresses command output to save LLM tokens"
  homepage "https://github.com/pointfiveinc/tokenshift"
  version "0.2.3"

  on_macos do
    if Hardware::CPU.intel?
      url "https://github.com/pointfiveinc/tokenshift/releases/download/v0.2.3/tokenshift_0.2.3_darwin_amd64.tar.gz",
        headers: [
          "Accept: application/octet-stream",
          "Authorization: bearer #{ENV["HOMEBREW_GITHUB_API_TOKEN"]}"
        ]
      sha256 "f94a034a675edc3aad16a7b2a06da2dc7b9c851eba2e99e124ea02b0d6697f88"

      def install
        bin.install "tokenshift"
      end
    end
    if Hardware::CPU.arm?
      url "https://github.com/pointfiveinc/tokenshift/releases/download/v0.2.3/tokenshift_0.2.3_darwin_arm64.tar.gz",
        headers: [
          "Accept: application/octet-stream",
          "Authorization: bearer #{ENV["HOMEBREW_GITHUB_API_TOKEN"]}"
        ]
      sha256 "edd85481c95c2fe5848cac7e8fd2f7a58b1008d328f1c750f64ee5671bd7418c"

      def install
        bin.install "tokenshift"
      end
    end
  end

  on_linux do
    if Hardware::CPU.intel?
      if Hardware::CPU.is_64_bit?
        url "https://github.com/pointfiveinc/tokenshift/releases/download/v0.2.3/tokenshift_0.2.3_linux_amd64.tar.gz",
          headers: [
            "Accept: application/octet-stream",
            "Authorization: bearer #{ENV["HOMEBREW_GITHUB_API_TOKEN"]}"
          ]
        sha256 "4a3a8fb3402dbe779ff73096f203cdf1e5d0af1d6e2f98f84bddb45c248dff8f"

        def install
          bin.install "tokenshift"
        end
      end
    end
    if Hardware::CPU.arm?
      if Hardware::CPU.is_64_bit?
        url "https://github.com/pointfiveinc/tokenshift/releases/download/v0.2.3/tokenshift_0.2.3_linux_arm64.tar.gz",
          headers: [
            "Accept: application/octet-stream",
            "Authorization: bearer #{ENV["HOMEBREW_GITHUB_API_TOKEN"]}"
          ]
        sha256 "37d9550102648343b5e698ab1f7412fa6b8c0499992843d92f25429564f57fd3"

        def install
          bin.install "tokenshift"
        end
      end
    end
  end

  test do
    system "#{bin}/tokenshift", "version"
  end
end
